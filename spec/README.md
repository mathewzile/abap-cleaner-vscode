...
The TypeScript core and VS Code extension are implemented and currently register 73 bounded cleanup rules (61 ABAP, 12 DDL/DCL). The core suite has 295 passing tests. Full parser/pretty-printer parity, the bulk of the ABAP/DDL `ALIGNMENT` groups (one rule, `ALIGN_ASSIGNMENTS`, is ported as a proof of concept), semantic analysis, most DDL section-classification rules, and the Rust replacement remain future work; `CAMEL_CASE_NAME`/`DDL_CAMEL_CASE_NAME` (identifier-casing lookups) are explicitly out of scope, not future work — see [08-roadmap.md](08-roadmap.md) "Current Status" for exactly what is and isn't ported and why. The phase documents below are retained as architecture and roadmap material; their full-parity milestones are not yet complete.
