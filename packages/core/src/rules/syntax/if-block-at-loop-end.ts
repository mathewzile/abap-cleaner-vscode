...
const LOOP_KINDS = new Set(['loop', 'do', 'while']);

export function replaceSimpleIfBlockAtLoopEnd(sourceText: string): string {
  const commands = parseAbapCommands(sourceText);
  const pairs = findControlBlockPairs(commands);
  const replacements: Replacement[] = [];

  for (const ifPair of pairs) {
    if (ifPair.kind !== 'if' || ifPair.closingCommandIndex !== ifPair.openingCommandIndex + 2) continue;
    const loopPair = pairs.find((pair) => LOOP_KINDS.has(pair.kind) && pair.closingCommandIndex === ifPair.closingCommandIndex + 1);
    const ifCommand = commands[ifPair.openingCommandIndex]!;
    const bodyCommand = commands[ifPair.openingCommandIndex + 1]!;
    const operand = simpleInitialIfOperand(ifCommand);
    if (loopPair === undefined || operand === undefined || bodyCommand.startLine !== bodyCommand.endLine || bodyCommand.tokens.some((token) => token.kind === 'comment')) continue;
    ...
    replacements.push({
      ...
      end: commands[ifPair.closingCommandIndex]!.endOffset,
      text: `IF ${operand} IS NOT INITIAL.${lineSeparator}${ifIndent}  CONTINUE.${lineSeparator}${ifIndent}ENDIF.${lineSeparator}${ifIndent}${bodyText}`,
    });
  }
